...
class LevelCog(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if not is_text_channel(message.channel):
            return
        ...
        if message.content.startswith("https://tenor.com/view/"):
            return
        elif len(COMBINED_PATTERN.sub("", message.content).strip()) <= 5:
            return
        ...
        if message.channel.id == config.channels.self_introduction_channel:  # 書き換えること
        ...
        server_booster = message.guild.get_role(config.roles.server_booster)
