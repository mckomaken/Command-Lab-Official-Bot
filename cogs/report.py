...
"""
        report_embed = discord.Embed(
            title="通報を受け付けました", description=REPORTMESSAGE, color=0x80FF00
        )
        report_embed.set_image(
            url=self.message.attachments[0].url if self.message.attachments else None
        )
...
"""
        report_log_embed = discord.Embed(
            title="通報がありました", description=REPORTLOGMESSAGE, color=0xFF00FF
        )
        report_log_embed.set_image(
            url=self.message.attachments[0].url if self.message.attachments else None
        )
...
class Report(commands.Cog):
    def __init__(self, bot: commands.Bot) -> None:
        ...
        self.context_menu = app_commands.ContextMenu(
            name="このメッセージを通報", callback=self.send_msg
        )
        ...
        options = Select(
            ...
            options=[
                discord.SelectOption(
                    label="内容が気に入らない(その他)", value="dont-like"
                ),
                discord.SelectOption(
                    label="Discordの各種規約違反", value="discord-violation"
                ),
                discord.SelectOption(
                    label="その他法令・規約違反", value="law-violation"
                ),
                discord.SelectOption(
                    label="チャンネルの趣旨に合わない", value="off-topic"
                ),
                discord.SelectOption(label="誹謗中傷・差別・脅迫", value="harass"),
                discord.SelectOption(
                    label="強い言葉づかい・否定的表現", value="negative-language"
                ),
                discord.SelectOption(label="暴力的・エロ・グロ", value="nsfw"),
                discord.SelectOption(label="荒らし・スパム", value="spam"),
                discord.SelectOption(
                    label="政治・宗教的活動", value="politics-religion"
                ),
                discord.SelectOption(
                    label="個人情報の過度な詮索・漏洩", value="exposing-information"
                ),
                discord.SelectOption(label="無意味なメンション", value="mention"),
                discord.SelectOption(
                    label="不適切な名前・画像・鯖タグ", value="inappropriate-profile"
                ),
                discord.SelectOption(
                    label="宣伝ルール違反", value="advertising-rule-violation"
                ),
                discord.SelectOption(
                    label="質問ルール違反", value="question-rule-violation"
                ),
                discord.SelectOption(
                    label="他参加者へのなりすまし", value="impersonation"
                ),
            ],
            min_values=1,
            max_values=1,
