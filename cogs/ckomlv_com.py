...
class Cmdbotlevelcom(commands.Cog):
    ...
    @app_commands.describe(
        target="ユーザー名", more_info="詳細情報の表示(初期設定: False(非表示)"
    )
    async def clevel(
        self,
        ...
        if target is None:
            ...
            if not userdb:
                ...
                if more_info is True:
                    gachadb = (
                        session2.query(Oregacha)
                        .filter_by(userid=interaction.user.id)
                        .first()
                    )
                    if not gachadb:
                        ...
                        gachaplus1 = (
                            gachadb.netheritei * 2200
                            + gachadb.netherites * 400
                            + gachadb.lapis * 180
                            + gachadb.diamond * 250
                            + gachadb.gold * 150
                            + gachadb.redstone * 130
                        )
                        gachaplus2 = (
                            gachadb.emerald * 100
                            + gachadb.iron * 85
                        ...
                        gachaminus = (
                            gachadb.breaking_pickaxe * 100
                            + gachadb.broken_pickaxe * 400
                            + gachadb.death * 1111
                        )
                        gachaplus91 = (
                            gachadb.beacon * 30000
                            + gachadb.netheriteb * 20000
                            + gachadb.lapisb * 1620
                            + gachadb.diamondb * 2250
                            + gachadb.goldb * 1350
                            + gachadb.redstoneb * 1170
                        )
                        gachaplus92 = (
                            gachadb.emeraldb * 900
                            + gachadb.ironb * 765
                            + gachadb.copperb * 360
                            + gachadb.quartzb * 220
                            + gachadb.coalb * 720
                        ...
                        gachaminus9 = (
                            gachadb.broken_pickaxe9 * 1000
                            + gachadb.death9 * 4000
                            + gachadb.unkownworld * 10000
                        )
                        level_embed.add_field(
                            name="総獲得経験値量",
                            value=f"```go\n{userdb.alladdexp} exp\n```",
            ...
            if not targetdb:
                await interaction.response.send_message(
                    f"`{target.display_name}`はまだ経験値を獲得していません\n### 喋らせよう!!!!!(笑)",
                    silent=True,
                )
                return
            elif targetdb.noxp is True or target.id == config.syunngikuid:
                await interaction.response.send_message(
                    f"`{target.display_name}`の経験値量は確認できません", ephemeral=True
                )
                return
            else:
                level_embed = discord.Embed(
    ...
    async def cgivexp(
        self, interaction: discord.Interaction, target: discord.Member, givexp: int
    ):
        givedb = session.query(User).filter_by(userid=interaction.user.id).first()
        targetdb = session.query(User).filter_by(userid=target.id).first()
        if givedb.dailygivexp is True:
            await interaction.response.send_message(
                "今日はもうあげられないよ", ephemeral=True
            )
            return
        if 0 >= givexp or givexp >= 5000:
            await interaction.response.send_message(
                "引数:givexpは1以上4999以下を指定してください", ephemeral=True
            )
            return
        if targetdb.noxp is True or givedb.noxp is True:
            await interaction.response.send_message(
                f"`{target.display_name}`に経験値を与えることはできません",
                ephemeral=True,
            )
            return
        if not givedb:
            await interaction.response.send_message(
                f"{interaction.user.mention}のデータがないため、そもそも与える経験値がありません\n喋ろう!!!!",
                silent=True,
            )
            return
        elif not targetdb:
            ...
            session.commit()
            await interaction.response.send_message(
                f"{target.mention}のデータがなかったため、只今作成しました\nもう一度コマンドを実行して付与してください",
                silent=True,
            )
            return
        ...
        if givedb_allexp < givexp:
            await interaction.response.send_message(
                f"コマ研レベルに借金機能はありません(笑)\n所持経験値量：{givedb_allexp} < 付与予定経験値量：{givexp}",
                silent=True,
            )
            return
        ...
        session.commit()
        await interaction.response.send_message(
            f"{target.mention}に{givexp}与えました", silent=True
        )

    @app_commands.command(
        name="csetleveling", description="【運営用】参加者のLv/exp変更)"
    )
    @app_commands.describe(
        choice="選択肢", target="変更する人", level="レベル", experience="経験値"
    )
    ...
    async def setleveling(
        ...
        if level == 0 and experience == 0:
            if choice.value != "stop" or choice.value != "list":
                await interaction.response.send_message(
                    "`level`または`experience`またはその両方に引数がありません\nどちらか一つは引数を指定してください",
                    silent=True,
                )
                return
        ...
        match choice.value:
            case "add":
                ...
                session.commit()
                await interaction.response.send_message(
                    f"{target.mention}に{level}Lv{experience}exp分を付与しました",
                    silent=True,
                )
            case "remove":
                if ((setuserdb.level * 10000) + setuserdb.exp) <= (
                    (level * 10000) + experience
                ):
                    setuserdb.exp = 0
                    setuserdb.level = 0
                    session.commit()
                    ...
                    session.commit()
                await interaction.response.send_message(
                    f"{target.mention}の{level}Lv{experience}exp分をはく奪しました",
                    silent=True,
                )
            case "set":
                if experience >= 10000:
                    await interaction.response.send_message(
                        "`experience`が10000以上のため、設定できません", silent=True
                    )
                    return
                setuserdb.exp = experience
                setuserdb.level = level
                session.commit()
                await interaction.response.send_message(
                    f"{target.mention}を{level}Lv{experience}expに設定しました",
                    silent=True,
                )
            case "stop":
                if setuserdb.noxp is False:
                    ...
                    session.commit()
                    await interaction.response.send_message(
                        f"{target.mention}のレベルシステムを無効化しました", silent=True
                    )
                else:
                    setuserdb.noxp = False
                    session.commit()
                    await interaction.response.send_message(
                        f"{target.mention}のレベルシステムを有効化しました", silent=True
                    )
            case "list":
                level_embed = discord.Embed(
                    title=f"{target.display_name}のレベル",
