...
class CUUIDCog(commands.Cog):
    ...
    @app_commands.command(name="cuuid", description="UUIDを生成します")
    @app_commands.describe(count="生成する量(デフォルト: 2)")
    async def cuuid(
        self,
        ...
        for i in range(count):
            ...
            uuid_embed.add_field(
                name=f"{i + 1}個目",
                value=f"{create_codeblock(uuid2)}{create_codeblock(uuid2_nosep)}",
                inline=False,
            )
