...
class CRandom(commands.Cog):
    ...
    @app_commands.command(
        name="crandom", description="マイクラの/randomコマンドの一部です(reset引数なし)"
    )
    ...
    async def crandom(
        ...
        if re.fullmatch(pattern1, range):
            ...
            if starts >= ends:
                await interaction.response.send_message(
                    "「[小さい数字]..[大きい数字]」って書いてね", ephemeral=True
                )
                return
            elif abs(starts - ends) + 1 > 2147483646:
                await interaction.response.send_message(
                    "範囲が広すぎます\n範囲の大きさは2以上2147483646以下です",
                    ephemeral=True,
                )
                return
            elif starts <= start:
                await interaction.response.send_message(
                    "[小さい数字]は-2147483648より大きい数で書いてね", ephemeral=True
                )
                return
            elif ends >= end:
                await interaction.response.send_message(
                    "[大きい数字]は2147483647より小さい数で書いてね", ephemeral=True
                )
                return
            ...
            if abs(starts - ends) + 1 > 2147483646:
                ...
                await interaction.response.send_message(
                    "[小さい数字]は2147483647より小さい数で書いてね", ephemeral=True
                )
                return
            ...
            if abs(start - ends) + 1 > 2147483646:
                ...
                await interaction.response.send_message(
                    "[大きい数字]は-2147483648より大きい数で書いてね", ephemeral=True
                )
                return
        ...
        if mode.value == "mroll":
            await interaction.response.send_message(
                f"{interaction.user.display_name}は{num}を引きました ( 値の範囲は{starts}から{ends} )",
                ephemeral=False,
                silent=True,
            )
