    ...
    async def on_message(self, message: discord.Message):
        if message.content.startswith("mee6level"):
            if message.channel.id in [
                ...
                mee6_channel = await self.bot.fetch_channel(
                    config.mee6.levelup
                )  # 新たに作るmee6通知チャンネル
                ...
                jointime = int(
                    datetime.now().timestamp() - lvupuser.joined_at.timestamp()
                )
                ...
                if level == 1 and levelupnoticeoff not in lvupuser.roles:
                    await mee6_channel.send(
                        f"{text}{icon} /xp reached <@{userid}> mee6-level {level}\n-# メンション通知がうるさいと感じたら<#892255648295841842>で`MEE6レベル無効化`ロールを付けてね"
                    )
                    return
                elif level >= 5:
                    if (
                        senndennkenn not in lvupuser.roles
                        and hanabira not in lvupuser.roles
                    ):
                        if jointime >= 86400:
                            await lvupuser.add_roles(senndennkenn)
                        else:
                            await admin_channel.send(
                                f"<@{userid}>：宣伝権(仮)ロールを{jointime1day}に付与してください"
                            )

                userdisp = (
                    f"`{username}`"
                    if levelupnoticeoff in lvupuser.roles
                    else f"<@{userid}>"
                )
                await mee6_channel.send(
                    f"{text}{icon} /xp reached {userdisp} mee6-level {level}"
                )
            ...
            if message.channel.id in [
                ...
                await mee6_channel.send(
                    f"{text}{icon} /xp reached {userdisp} mcmd-level {level}"
                )
