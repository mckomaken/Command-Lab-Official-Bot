...
class CItem(commands.Cog):
    ...
    async def citem(self, interaction: discord.Interaction, id: str):
        async with aiofiles.open("./minecraft_data/data/dataPaths.json") as fp:
            ...
            async with aiofiles.open("./minecraft_data/data/" + dataPath.pc[config.latest_version].items + "/items.json") as fp:
                async with aiofiles.open("./minecraft_data/data/" + dataPath.pc[config.latest_version].blocks + "/blocks.json") as fp2:
                    ...
                    for item in items.root:
                        if item.name == id.replace("minecraft:", ""):
                            ...
                            block = next(iter([b for b in blocks.root if b.name == item.name]), None)

                            async with aiofiles.open("./tmp/ja_jp.json", mode="rb") as lang_fp:
                                with zipfile.ZipFile(f"./tmp/client_{config.latest_version}.jar") as zipfp:
                                    typename = "item" if is_item else "block"
                                    lang_data = json.loads(await lang_fp.read())
                                    lang_text = lang_data[f"{typename}.minecraft.{item.name}"]
                                    with zipfp.open(f"assets/minecraft/textures/{typename}/{id}.png") as imgfp:
                                        img = Image.open(imgfp).resize((256, 256), Image.Resampling.NEAREST)
                                        streamimg = io.BytesIO()
                                        img.save(streamimg, "WEBP")
                                        file = discord.File(io.BytesIO(streamimg.getvalue()), filename=f"{id}.webp")
                                        files = [file]
                                        embed = discord.Embed(
                                            title=lang_text,
                                        ...
                                        embed.add_field(name="最大スタック数", value=create_codeblock(f"{item.stackSize}"))

                                        if block is not None:
                                            if block.boundingBox != "empty":
                                                embed.add_field(name="爆破耐性", value=create_codeblock(block.resistance))
                                                embed.add_field(name="硬度", value=create_codeblock(block.hardness))

                                            if block.material == "mineable/pickaxe":
                                                embed.add_field(name="適正ツール", value=create_codeblock("ピッケル"))
                                            elif block.material == "mineable/axe":
                                                embed.add_field(name="適正ツール", value=create_codeblock("斧"))
                                            elif block.material == "mineable/shovel":
                                                embed.add_field(name="適正ツール", value=create_codeblock("シャベル"))
                                            elif block.material == "mineable/hoe":
                                                embed.add_field(name="適正ツール", value=create_codeblock("クワ"))
                                            elif block.material == "wool":
                                                embed.add_field(name="適正ツール", value=create_codeblock("ハサミ"))
                                            elif block.material == "coweb":
                                                embed.add_field(name="適正ツール", value=create_codeblock("剣"))
                                            else:
                                                embed.add_field(name="適正ツール", value=create_codeblock("素手"))

                                            di_imgs = Image.new("RGBA", (1000, 64), 0x000000FF)
                                            for d in block.drops:
                                                if drop_item := next(iter([di for di in items.root if di.id == d]), None):
                                                    ...
                                                    with zipfp.open(f"assets/minecraft/textures/{di_typename}/{drop_item.name}.png") as imgfp2:
                                                        ci += 68
                                                        di_imgs.paste(Image.open(imgfp2).resize((64, 64), Image.Resampling.NEAREST), (ci, 0))

                                            di_imgs_stream = io.BytesIO()
                                            di_imgs.save(di_imgs_stream, "WEBP")
                                            file2 = discord.File(io.BytesIO(di_imgs_stream.getvalue()), filename=f"{id}_loot.webp")
                                            files.append(file2)
