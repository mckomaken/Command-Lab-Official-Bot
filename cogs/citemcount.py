...
class Itemcount_I2S(discord.ui.Modal):
    def __init__(self) -> None:
        ...
        self.item = discord.ui.TextInput(
            label="アイテム数", placeholder="数字で入力してください"
        )
...
class Itemcount_S2I(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        if (
            not self.item.value.isdecimal()
            or not self.stack.value.isdecimal()
            or not self.schest.value.isdecimal()
            or not self.lchest.value.isdecimal()
        ):
            await interaction.response.send_message(
                "数字で入力してください", ephemeral=True
            )
        ...
        if sum <= 64:
            ...
            await interaction.response.send_message(
                f"2スタック以内ぐらいは暗算してくれ......w\n一応、{sum}アイテムやで",
                ephemeral=True,
            )
        else:
            await interaction.response.send_message(
                f"{lchests}LC+{schests}c+{stacks}s+{items}i = {sum}i"
            )
...
class CItemcount(commands.Cog):
    ...
    @app_commands.command(
        name="citemnum-change", description="実行するとアイテム数を変換してくれます"
    )
    @app_commands.describe(choice="選択肢")
    @app_commands.choices(
        choice=[
            app_commands.Choice(
                value="i2s", name="アイテム数→スタック数等(10000i→2LC+1c+21st+16i)"
            ),
            app_commands.Choice(
                value="s2i", name="スタック数等→アイテム数(2LC+1c+21st+16i→10000i)"
            ),
        ]
    )
    async def itemcount(
