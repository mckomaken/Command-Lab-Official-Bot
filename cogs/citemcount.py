...
class Itemcount_I2S(discord.ui.Modal):
    def __init__(self) -> None:
        super().__init__(title="【数値変換】アイテム数→スタック数等", timeout=None, custom_id="itos-modal")

        self.item = discord.ui.TextInput(label="アイテム数", placeholder="数字で入力してください")
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        if items <= 64:
            ...
            await interaction.response.send_message(
                f"{items}items = {lc}LC+{sc}c+{st}s+{stmod}i\n-# = {lc}ラージチェスト+{sc}チェスト+{st}スタック+{stmod}アイテム\n-# = {lc * 2 + sc}シュルカーボックス+{st}スタック+{stmod}アイテム"
            )


class Itemcount_S2I(discord.ui.Modal):
    def __init__(self) -> None:
        super().__init__(title="【数値変換】スタック数等→アイテム数", timeout=None, custom_id="stoi-modal")

        self.item = discord.ui.TextInput(label="i-アイテム数", placeholder="数字で入力してください", required=False, default="0", row=3)
        self.stack = discord.ui.TextInput(label="st-スタック数", placeholder="数字で入力してください", required=False, default="0", row=2)
        self.schest = discord.ui.TextInput(label="c-スモールチェスト数(シュル箱数)", placeholder="数字で入力してください", required=False, default="0", row=1)
        self.lchest = discord.ui.TextInput(label="lc-ラージチェスト数", placeholder="数字で入力してください", required=False, default="0", row=0)
...
class CItemcount(commands.Cog):
    ...
    @app_commands.choices(
        choice=[app_commands.Choice(value="i2s", name="アイテム数→スタック数等(10000i→2LC+1c+21st+16i)"), app_commands.Choice(value="s2i", name="スタック数等→アイテム数(2LC+1c+21st+16i→10000i)")]
    )
