...
class UrlView(discord.ui.View):
    ...
    async def convert_url(
        ...
        await interaction.response.send_message(
            embed=create_embed(title="送信内容", description=(f"{self.url}")),
            ephemeral=True,
        )
...
class CYbase64(commands.Cog):
    ...
    @app_commands.describe(
        url="URLを書いてください(変換されます)",
        text="送信したい文章を書いてください(任意・変換されません)",
    )
    async def cybase64(
        self, interaction: discord.Interaction, url: str, text: str = None
    ):
