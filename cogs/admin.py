...
class LotteryView(discord.ui.View):  # 抽選コマンド
    ...
    async def lottery_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        assert isinstance(interaction.channel, discord.abc.Messageable)
        assert config.channels.lottery_channel is not None

        send_channel = await self.bot.fetch_channel(config.channels.lottery_channel)
        assert isinstance(send_channel, discord.abc.Messageable)
...
class AdminCog(commands.Cog):
    ...
    async def cn(
        ...
        notice_embed = discord.Embed(title=title, description=description, color=0xFF580F, timestamp=current_time)
        ...
        await interaction.response.send_message(
            ...
            embed=notice_embed,
            ephemeral=True,
            view=NoticeConfirmView(embed=notice_embed),
        )
