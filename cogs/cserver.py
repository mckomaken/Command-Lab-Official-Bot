...
def server_embed(ip:str, is_java:bool):
    ...
    class LikeButton(discord.ui.Button):
        ...
        async def callback(self, interaction: discord.Interaction):
                ...
                if not raw:
                    cur.execute(f'insert into server values({interaction.user.id}, "dict()")')
                    cur.execute(f"select ips from server where userid = {interaction.user.id}")
                    raw = cur.fetchall()
...
class CServer(commands.Cog):
    ...
    async def clist(self, interaction:discord.Interaction):
        ...
        # まだデータベースにユーザーidがなかった場合
        if not raw:
            await interaction.followup.send(f"お気に入り登録されているサーバーはありません", ephemeral=True)
            return
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                class RemoveSelect(discord.ui.Select):
                    async def callback(self, interaction: discord.Interaction):
                        ...
                        await interaction.response.send_message(f"`{target}`をお気に入りから解除しました", ephemeral=True)
                ...
                select = RemoveSelect(
                    ...
                    min_values=1,
                    max_values=1,
                    options=[discord.SelectOption(label=server) for server in self.serverlist]
                    )
