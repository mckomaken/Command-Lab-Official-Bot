...
class Cell(Area):
    ...
    def is_painted(self, curve: Curve) -> bool:
        ...
        is_right_top = lambda x, y: is_right(x) and is_top(y)  # noqa(←pep8警告無視)
        is_right_bottom = lambda x, y: is_right(x) and is_bottom(y)  # noqa
        is_left_top = lambda x, y: is_left(x) and is_top(y)  # noqa
        is_left_bottom = lambda x, y: is_left(x) and is_bottom(y)  # noqa
