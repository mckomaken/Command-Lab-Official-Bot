...
class IntegerArgumentType(ArgumentType[int]):
    ...
    def parse(self, reader: StringReader) -> int:
        ...
        if result < self.minimum:
            ...
            raise BUILT_IN_EXCEPTIONS.integer_too_low().createWithContext(reader, result, self.minimum)
        if result > self.maximum:
            reader.setCursor(start)
            raise BUILT_IN_EXCEPTIONS.integer_too_high().createWithContext(reader, result, self.maximum)
