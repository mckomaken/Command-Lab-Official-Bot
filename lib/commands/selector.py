...
class Selector(BaseModel):
    target: Literal[
        "this", "all_entities", "all_players", "random_player", "nearest_player"
    ]
        ...
        if not self.reader.canRead():
            ...
            if c == "p":
                ...
                if c != "e":
                    ...
                    UNKNOWN_SELECTOR_EXCEPTION.createWithContext(
                        self.reader, "@" + str(c)
                    )
...
class EntitySelectorOptions:
    ...
    def get_handler(reader: EntitySelectorReader, string: str, i: int):
        ...
        if selector_option is not None:
            ...
            raise UNKNOWN_OPTION_EXCEPTION.createWithContext(
                reader.get_reader(), string
            )
    ...
    def init(self):
        ...
        class LevelOption(SelectorHandler):
            def handle(self, reader: EntitySelectorReader):
                ...
                if (intRange is None or intRange.min >= 1) and (
                    ...
                    reader.get_reader().setCursor(i)
                    raise NEGATIVE_LEVEL_EXCEPTION.createWithContext(
                        reader.get_reader()
                    )
        ...
        self.put_option(
            "x_rotation",
            XRotationOption(),
            Predicate(lambda reader: FloatRnage.any().test(reader.pitchRange)),
            Text.translatable("argument.entity.options.x_rotation.description"),
        )
        self.put_option(
            "y_rotation",
            YRotationOption(),
            Predicate(lambda reader: FloatRnage.any().test(reader.yawRange)),
                    ...
                    if reader.read_tag_character():
                        ...
                        if entity_type is None:
                            ...
                            return INVALID_TYPE_EXCEPTION.createWithContext(
                                reader.get_reader(), str(identifier)
                            )
